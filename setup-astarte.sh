...
cd $tmp_dir
wget -q https://github.com/astarte-platform/astartectl/releases/download/v1.0.0/astartectl_1.0.0_linux_x86_64.tar.gz
tar xf astartectl_1.0.0_linux_x86_64.tar.gz
chmod +x astartectl
cd -
...
astartectl cluster instances deploy --version "$1" --api-host "api.autotest.astarte-platform.org" --broker-host "broker.autotest.astarte-platform.org" \
    ...
    --cassandra-volume-size "4G" --name "astarte" --namespace "$2" -y || exit 1
